  ...
  async createCompany(organizationId: string, input: CreatePharmacyCompany) {
    const [row] = await this.db
      ...
      .values({
        ...
        companyTitle: input.companyTitle?.trim() || null,
        address2: input.address2?.trim() || null,
        bankName: input.bankName?.trim() || null,
        bankBranch: input.bankBranch?.trim() || null,
        transport: input.transport?.trim() || null,
        salesmanCommissionPct: Math.round(input.salesmanCommissionPct ?? 0),
        showForAdvTax: input.showForAdvTax?.trim() || null,
        productPolicyJson: input.productPolicyJson?.trim() || null,
        discountPolicyJson: input.discountPolicyJson?.trim() || null,
        companyShiftingJson: input.companyShiftingJson?.trim() || null,
  ...
  async createTradeCustomer(organizationId: string, input: CreatePharmacyTradeCustomer) {
    ...
    const [row] = await this.db
      ...
      .values({
        ...
        oldCode: input.oldCode?.trim() || null,
        partyType: input.partyType?.trim() || null,
        speciality: input.speciality?.trim() || null,
        storeType: input.storeType?.trim() || null,
        contactPerson: input.contactPerson?.trim() || null,
        nicNumber: input.nicNumber?.trim() || null,
        ntnNumber: input.ntnNumber?.trim() || null,
        passportNo: input.passportNo?.trim() || null,
        landLine: input.landLine?.trim() || null,
        customerClass: input.customerClass?.trim() || null,
        detailedAddress: input.detailedAddress?.trim() || null,
        cityName: input.cityName?.trim() || null,
        regDate: input.regDate?.trim() || null,
        creditCategory: input.creditCategory?.trim() || null,
        monthlySalesTargetPkr: Math.round(input.monthlySalesTargetPkr ?? 0),
        modeOfPayment: input.modeOfPayment?.trim() || null,
        timing: input.timing?.trim() || null,
        visitFrequency: Math.round(input.visitFrequency ?? 0),
        visitingDaysJson: input.visitingDays ? JSON.stringify(input.visitingDays) : null,
        termsOfPayment: input.termsOfPayment?.trim() || null,
        license9No: input.license9No?.trim() || null,
        license9Expiry: input.license9Expiry?.trim() || null,
        license10No: input.license10No?.trim() || null,
        license10Expiry: input.license10Expiry?.trim() || null,
        license11No: input.license11No?.trim() || null,
        license11Expiry: input.license11Expiry?.trim() || null,
        advTaxType: input.advTaxType?.trim() || null,
        isFocCustomer: input.isFocCustomer ?? false,
        canDeductTax: input.canDeductTax ?? false,
        taxPct: Math.round(input.taxPct ?? 0),
        imageUrl: input.imageUrl?.trim() || null,
        discountPolicyJson: input.discountPolicyJson?.trim() || null,
        bonusPolicyJson: input.bonusPolicyJson?.trim() || null,
        companyCreditLimitsJson: input.companyCreditLimitsJson?.trim() || null,
        companyIdsJson: input.companyIdsJson?.trim() || null,
        blockSalesJson: input.blockSalesJson?.trim() || null,
        othersJson: input.othersJson?.trim() || null,
        locationLat: input.locationLat?.trim() || null,
        locationLng: input.locationLng?.trim() || null,
        locationNotes: input.locationNotes?.trim() || null,
        accountType: input.accountType?.trim() || null,
        companyCode: input.companyCode?.trim() || null,
        uniqueName: input.uniqueName?.trim() || null,
        postalAddress: input.postalAddress?.trim() || null,
        province: input.province?.trim() || null,
        fax: input.fax?.trim() || null,
        partyMode: input.partyMode?.trim() || null,
        stxNo: input.stxNo?.trim() || null,
        sector: input.sector?.trim() || null,
        licenceNo: input.licenceNo?.trim() || null,
        licenceExpiry: input.licenceExpiry?.trim() || null,
        activeTaxPayer: input.activeTaxPayer ?? false,
        incomeTaxExempt: input.incomeTaxExempt ?? false,
        advanceTaxSummary: input.advanceTaxSummary ?? false,
        invoiceWarranty: input.invoiceWarranty ?? false,
        status: input.status?.trim() || "active",
      })
      .returning();
