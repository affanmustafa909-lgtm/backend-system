...
export const pharmacyCompanies = pgTable("pharmacy_companies", {
  ...
  /** Legacy Company Target form fields. */
  companyTitle: text("company_title"),
  address2: text("address_2"),
  bankName: text("bank_name"),
  bankBranch: text("bank_branch"),
  transport: text("transport"),
  salesmanCommissionPct: integer("salesman_commission_pct").notNull().default(0),
  showForAdvTax: text("show_for_adv_tax"),
  productPolicyJson: text("product_policy_json"),
  discountPolicyJson: text("discount_policy_json"),
  companyShiftingJson: text("company_shifting_json"),
...
export const pharmacyTradeCustomers = pgTable("pharmacy_trade_customers", {
  ...
  /** Legacy Dist Customer Information form (desktop Company master parity). */
  ...
  timing: text("timing"),
  visitFrequency: integer("visit_frequency").notNull().default(0),
  visitingDaysJson: text("visiting_days_json"),
  termsOfPayment: text("terms_of_payment"),
  license9No: text("license_9_no"),
  license9Expiry: date("license_9_expiry"),
  license10No: text("license_10_no"),
  license10Expiry: date("license_10_expiry"),
  license11No: text("license_11_no"),
  license11Expiry: date("license_11_expiry"),
  advTaxType: text("adv_tax_type"),
  isFocCustomer: boolean("is_foc_customer").notNull().default(false),
  canDeductTax: boolean("can_deduct_tax").notNull().default(false),
  taxPct: integer("tax_pct").notNull().default(0),
  imageUrl: text("image_url"),
  discountPolicyJson: text("discount_policy_json"),
  bonusPolicyJson: text("bonus_policy_json"),
  companyCreditLimitsJson: text("company_credit_limits_json"),
  companyIdsJson: text("company_ids_json"),
  blockSalesJson: text("block_sales_json"),
  othersJson: text("others_json"),
  locationLat: text("location_lat"),
  locationLng: text("location_lng"),
  locationNotes: text("location_notes"),
  /** Chart of Account → Client tab fields (legacy CoA client master). */
  ...
  fax: text("fax"),
  partyMode: text("party_mode"),
  stxNo: text("stx_no"),
  sector: text("sector"),
  licenceNo: text("licence_no"),
  licenceExpiry: date("licence_expiry"),
  activeTaxPayer: boolean("active_tax_payer").notNull().default(false),
  incomeTaxExempt: boolean("income_tax_exempt").notNull().default(false),
  advanceTaxSummary: boolean("advance_tax_summary").notNull().default(false),
  invoiceWarranty: boolean("invoice_warranty").notNull().default(false),
  status: text("status").notNull().default("active"),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, (t) => [
